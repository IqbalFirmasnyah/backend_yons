...
export class PaketWisataLuarKotaService {
  ...
  public async uploadImages(id: number, files: Express.Multer.File[]) {
    ...
    try {
      ...
      const existingImages: string[] =
        (travelPackage.fotoPaketLuar as string[]) || [];
      ...
      if (files.length > 0) {
        for (const file of files) {
          try {
            ...
            console.error(
              `Failed to unlink file during rollback: ${file.path}`,
              unlinkError,
            );
          }
        }
      ...
      if (error instanceof NotFoundException) {
        throw new NotFoundException(
          `Paket wisata Luar Kota with ID ${id} not found`,
        );
      }
  ...
  async createPaketWisataLuarKota(
    dto: CreatePaketWisataLuarKotaDto,
  ): Promise<PaketWisataLuarKotaResponseDto> {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          throw new BadRequestException(
            'A package with this name or unique field already exists.',
          );
        }
        throw new BadRequestException(
          `Failed to create paket wisata luar kota: ${error.message}`,
        );
      }
      throw error;
    }
    ...
    try {
      ...
      for (const imageName of imagesToDelete) {
        ...
        if (fs.existsSync(imagePath)) {
          ...
          console.log(
            `Deleted associated image on package removal: ${imagePath}`,
          );
        }
      }
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        ...
        if (error.code === 'P2003') {
          throw new BadRequestException(
            'Cannot delete paket wisata luar kota. It has related records.',
          );
        }

        throw new BadRequestException(
          `Failed to delete paket wisata luar kota: ${error.message}`,
        );
      }
  ...
  async updatePaketWisataLuarKota(
    id: number,
    dto: UpdatePaketWisataLuarKotaDto,
  ): Promise<PaketWisataLuarKotaResponseDto> {
    const { detailRute, pilihTanggal, ...paketData } = dto;
    ...
    try {
      ...
      const updatedPaket = await this.prisma.$transaction(async (prisma) => {
        ...
        const updated = await prisma.paketWisataLuarKota.update({
          ...
          data: {
            ...
            hargaEstimasi:
              paketData.hargaEstimasi !== undefined
                ? new Prisma.Decimal(paketData.hargaEstimasi)
                : undefined,

            // Convert pilihTanggal to Date if provided

            pilihTanggal: pilihTanggal
              ? new Date(pilihTanggal)
              : existingPaket.pilihTanggal,
          },
        });
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        ...
        throw new BadRequestException(
          `Failed to update paket wisata luar kota: ${error.message}`,
        );
      }
  ...
  private mapToResponseDto(paket: any): PaketWisataLuarKotaResponseDto {
    const baseDate =
      paket.pilihTanggal instanceof Date
        ? paket.pilihTanggal
        : new Date(paket.pilihTanggal);

    const tanggalMulaiWisata = baseDate;
    const tanggalSelesaiWisata = new Date(baseDate);
    tanggalSelesaiWisata.setDate(
      tanggalSelesaiWisata.getDate() + paket.estimasiDurasi,
    );
