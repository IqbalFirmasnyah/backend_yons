import {
  ...
  export class RescheduleService {
    ...
    public validateRescheduleEligibility(
      ...
      const daysToOldStart = this.diffDaysUTC(booking.tanggalMulaiWisata, tanggalPengajuan);
      const MIN_H_MINUS = 4;
    ...
    async validateOnly(bookingId: number, tanggalBaruIso: string, userId: number) {
      ...
      if (booking.userId !== userId) {
        throw new UnauthorizedException('You are not authorized to validate this booking.');
      }
      if (
