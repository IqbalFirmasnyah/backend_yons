...
export class ReportService {
   ...
   * BOOKING REPORT
   * =======================================================*/
  async generateBookingsReport(period: Period): Promise<{
    summary: ReportSummary;
    rows: ReportRow[];
  }> {
    let list: BookingWithRelations[] = [];
    if (typeof (this.bookingService as any).findAllBookingsForReport === 'function') {
      list = await (this.bookingService as any).findAllBookingsForReport({
        from: period.from,
        to: period.to,
      });
    } else if (typeof (this.bookingService as any).findAllBookings === 'function') {
      list = await (this.bookingService as any).findAllBookings();
    } else {
      // fallback kalau nama method berbeda
      list = await (this.bookingService as any).findAllBooking?.() ?? [];
    }

    const filtered: BookingWithRelations[] = list.filter((b: any) => {
    ...
    const rows: ReportRow[] = filtered.map((b: any) => {
      const produk =
        b.paket?.namaPaket ??
  ...
  renderBookingsHtml(payload: { summary: ReportSummary; rows: ReportRow[] }): string {
