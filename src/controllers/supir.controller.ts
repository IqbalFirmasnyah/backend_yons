import { 
  ...
  import { ApiTags, ApiOperation, ApiResponse as SwaggerApiResponse, ApiBearerAuth, ApiParam } from '@nestjs/swagger'; // Import Swagger decorators
  ...
  export class SupirController {
    ...
    @SwaggerApiResponse({ status: HttpStatus.CREATED, description: 'The driver has been successfully created.' }) // Swagger response
    @SwaggerApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid input data.' })
    @SwaggerApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized' })
...
@ApiOperation({ summary: 'Retrieve available drivers for a given date range' })
@SwaggerApiResponse({ status: HttpStatus.OK, description: 'Successfully retrieved available drivers.' })
@SwaggerApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid date range.' })
async availableSupir(
  ...
  return {
    ...
    @SwaggerApiResponse({ status: HttpStatus.OK, description: 'Successfully retrieved all drivers.' })
    @SwaggerApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized' })
    async findAll() {
      const data = await this.supirService.findAll();
      return {
        message: 'Daftar supir berhasil diambil',
        data,
      };
    }
    ...
    @ApiParam({ name: 'id', type: 'number', description: 'ID of the driver to retrieve' })
    @SwaggerApiResponse({ status: HttpStatus.OK, description: 'Successfully retrieved the driver.' })
    @SwaggerApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Driver not found.' })
    @SwaggerApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized' })
    async findOne(@Param('id', ParseIntPipe) id: number) { // Use ParseIntPipe for ID validation
    ...
    @ApiParam({ name: 'id', type: 'number', description: 'ID of the driver to update' })
    @SwaggerApiResponse({ status: HttpStatus.OK, description: 'The driver has been successfully updated.' })
    @SwaggerApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Driver not found.' })
    @SwaggerApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid input data.' })
    @SwaggerApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized' })
    async update(@Param('id', ParseIntPipe) id: number, @Body() updateSupirDto: UpdateSupirDto) { // Use ParseIntPipe
    ...
    @ApiParam({ name: 'id', type: 'number', description: 'ID of the driver to delete' })
    @SwaggerApiResponse({ status: HttpStatus.OK, description: 'The driver has been successfully deleted.' })
    @SwaggerApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Driver not found.' })
    @SwaggerApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized' })
    async delete(@Param('id', ParseIntPipe) id: number) { // Use ParseIntPipe
